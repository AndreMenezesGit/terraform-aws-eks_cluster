data "aws_ami" "void-eks-worker" {
  filter {
    name   = "name"
    values = ["amazon-eks-node-${aws_eks_cluster.void-eks.version}-v*"]
  }
...
locals {
  void-node-userdata = <<USERDATA
    ...
    --apiserver-endpoint '${aws_eks_cluster.void-eks.endpoint}' \
    --b64-cluster-ca '${aws_eks_cluster.void-eks.certificate_authority.0.data}' \
    '${var.cluster_name}'
    USERDATA
}
...
resource "aws_launch_configuration" "void-eks-worker" {
...
resource "aws_autoscaling_group" "void-eks-worker" {
  ...
  launch_configuration = aws_launch_configuration.void-eks-worker.id
